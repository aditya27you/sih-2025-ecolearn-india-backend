...
export const getAllActiveChallenges = catchAsync(async (req: Request, res: Response) => {
...
export const getChallengeById = catchAsync(async (req: Request, res: Response) => {
...
export const submitChallengeProof = catchAsync(async (req: Request, res: Response) => {
  if (!req.file) {
  ...
  res.status(201).json(new ApiResponse(201, submission, 'Challenge proof submitted successfully'));
});

export const getSubmissionsByChallenge = catchAsync(async (req: Request, res: Response) => {
  const submissions = await challengeService.getSubmissionsByChallenge(req.params.id);
  res.status(200).json(new ApiResponse(200, submissions, 'Submissions fetched successfully'));
});

export const approveSubmission = catchAsync(async (req: Request, res: Response) => {
  const { feedback } = req.body;
  const submission = await challengeService.approveSubmission(req.params.id, feedback);
  res.status(200).json(new ApiResponse(200, submission, 'Submission approved successfully'));
});

export const rejectSubmission = catchAsync(async (req: Request, res: Response) => {
  const { feedback } = req.body;
  if (!feedback) {
    throw new ApiError('Feedback is required for rejection', 400);
  }
  const submission = await challengeService.rejectSubmission(req.params.id, feedback);
  res.status(200).json(new ApiResponse(200, submission, 'Submission rejected successfully'));
});
